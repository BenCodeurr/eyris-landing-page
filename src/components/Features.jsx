...
function Features(props) {
  ...
  const textContainerClasses = isSecond ? 'flex justify-end gap-24' : 'flex justify-center gap-24';

  const roundedFullClasses = isSecond || isThird ? 'border rounded-full bg-black w-4 h-4' : 'border rounded-full bg-white w-4 h-4';

  const imageClasses = isThird
    ? 'w-[400px] h-[200px] object-cover ml-auto'
    : `w-[400px] h-[200px] object-cover ${isSecond ? 'transform scaleX(-1)' : ''}`;
